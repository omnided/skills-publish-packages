# Звіт з виконання практичної роботи: Publish Packages та Docker

## 1. Підготовка середовища (Prerequisites)

Було встановлено та налаштовано **Docker Desktop** для забезпечення кросплатформної сумісності та можливості запуску контейнерів.
- Перевірено роботу Docker Engine.
- Використано термінал (PowerShell/Bash) для виконання команд.

> **[Скріншот запущеного вікна Docker Desktop]**
![](img/изображение_2025-12-26_080427994.png)

---

## 2. Авторизація в GitHub Container Registry

Для завантаження образів було виконано вхід у реєстр `ghcr.io`.

1.  Згенеровано **Personal Access Token (Classic)** з правами:
    - `repo` (all)
    - `write:packages`
    - `read:packages`
![](img/изображение_2025-12-30_121325943.png)
2.  Виконано команду логіну в терміналі:

```bash
docker login ghcr.io -u omnided
```
В якості паролю використано згенерований токен.

Результат: Отримано повідомлення Login Succeeded.

## 3. Завантаження Docker-образу (Pull)
Виконано завантаження образу з пакетів репозиторію GitHub.

Використана команда:

```Bash
docker pull ghcr.io/omnided/publish-packages/game:sha-43ebee1
```
Результат: Термінал відобразив статус завантаження (Status: Downloaded newer image...).

> **[Скріншот терміналу після виконання команди pull та з успішним логіном]**
![](img/изображение_2025-12-26_080427994.png)
## 4. Запуск контейнера (Run)
Перевірено наявність образу командою docker image ls.

Запущено контейнер на порту 8080 у фоновому режимі:

```Bash

docker run -dp 8080:80 --rm ghcr.io/omnided/publish-packages/game:game:sha-43ebee1
```
Результат:

Команда повернула хеш-ідентифікатор контейнера.

У браузері за адресою http://localhost:8080 відкрито розгорнутий веб-додаток (гру).

> **[Скріншот терміналу з командою запуску та командою docker image ls]**
![](img/изображение_2025-12-26_081526666.png)
> 
> **[Скріншот відкритого локалхосту (гри) у браузері]**
![](img/изображение_2025-12-26_081558621.png)
